    ...
    public class AdamoController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateMapInitial([FromBody] Models.DTOs.CreateMapInitialRequest request)
        {
            ...
            try
            {
                ...
                // Auto-populate createdBy if not provided
                var createdBy = string.IsNullOrWhiteSpace(request.CreatedBy) ? "MAPTOOL" : request.CreatedBy;

                // UPSERT: Check if GR_NUMBER already exists
                var existing = await _adamoContext.MapInitials
                    .FirstOrDefaultAsync(m => m.GrNumber == request.GrNumber);
                
                bool isUpdate = false;
                Models.Adamo.MapInitial mapInitial;

                if (existing != null)
                {
                    // UPDATE existing record
                    isUpdate = true;
                    existing.EvaluationDate = request.EvaluationDate ?? existing.EvaluationDate;
                    existing.Chemist = request.Chemist ?? existing.Chemist;
                    existing.Assessor = request.Assessor ?? existing.Assessor;
                    existing.Dilution = request.Dilution ?? existing.Dilution;
                    existing.EvaluationSite = request.EvaluationSite ?? existing.EvaluationSite;
                    existing.Odor0H = request.Odor0H ?? existing.Odor0H;
                    existing.Odor4H = request.Odor4H ?? existing.Odor4H;
                    existing.Odor24H = request.Odor24H ?? existing.Odor24H;
                    existing.Comments = request.Comments ?? existing.Comments;
                    existing.LastModifiedDate = DateTime.UtcNow;
                    existing.LastModifiedBy = createdBy;

                    mapInitial = existing;
                    _adamoContext.MapInitials.Update(existing);
                }
                else
                {
                    // INSERT new record
                    mapInitial = new Models.Adamo.MapInitial
                    {
                ...
                _logger.LogInformation("{Action} MAP_INITIAL record for GR_NUMBER {GrNumber} with ID {Id}", 
                    isUpdate ? "Updated" : "Created", mapInitial.GrNumber, mapInitial.MapInitialId);

                Console.WriteLine($"✓ {(isUpdate ? "Updated" : "Created")} MAP_INITIAL: {mapInitial.GrNumber} (ID: {mapInitial.MapInitialId})");

                return isUpdate 
                    ? Ok(new 
                    { 
                        status = "success", 
                        message = "MAP_INITIAL record updated successfully",
                        table = "MAP_INITIAL",
                        isUpdate = true,
                        data = mapInitial 
                    })
        ...
        /// <summary>
        /// Create or update MAP_SESSION record (UPSERT)
        ...
        public async Task<IActionResult> CreateMapSession([FromBody] Models.DTOs.CreateMapSessionRequest request)
        {
            ...
            try
            {
                ...
                // UPSERT logic: Check if session with SessionLink exists
                Models.Adamo.MapSession? mapSession = null;
                bool isUpdate = false;

                if (!string.IsNullOrWhiteSpace(request.SessionLink))
                {
                    mapSession = await _adamoContext.MapSessions
                        .FirstOrDefaultAsync(s => s.SessionLink == request.SessionLink);
                    
                    if (mapSession != null)
                    {
                        isUpdate = true;
                        
                        // Update existing session (only update non-null values)
                        if (request.Stage != null) mapSession.Stage = request.Stage;
                        if (request.EvaluationDate.HasValue) mapSession.EvaluationDate = request.EvaluationDate;
                        if (request.Region != null) mapSession.Region = request.Region;
                        if (request.Segment != null) mapSession.Segment = request.Segment;
                        if (request.Participants != null) mapSession.Participants = request.Participants;
                        if (request.ShowInTaskList != null) 
                            mapSession.ShowInTaskList = request.ShowInTaskList.ToUpper();
                        if (request.SubStage.HasValue) mapSession.SubStage = request.SubStage;
                        if (request.Category != null) mapSession.Category = request.Category;
                        
                        mapSession.LastModifiedDate = DateTime.UtcNow;
                        mapSession.LastModifiedBy = createdBy;
                        
                        _adamoContext.MapSessions.Update(mapSession);
                    }
                }
                ...
                if (mapSession == null)
                {
                    mapSession = new Models.Adamo.MapSession
                    {
                        Stage = request.Stage,
                        EvaluationDate = request.EvaluationDate,
                ...
                _logger.LogInformation("{Action} MAP_SESSION with ID {SessionId}, SessionLink: {SessionLink}, Stage: {Stage}", 
                    isUpdate ? "Updated" : "Created", mapSession.SessionId, mapSession.SessionLink, mapSession.Stage);

                Console.WriteLine($"✓ {(isUpdate ? "Updated" : "Created")} MAP_SESSION: ID {mapSession.SessionId} - {mapSession.Stage} - Link: {mapSession.SessionLink}");

                var statusCode = isUpdate ? 200 : 201;
                var message = isUpdate ? "MAP_SESSION record updated successfully" : "MAP_SESSION record created successfully";
            ...
            {
                _logger.LogError(ex, "Database error creating/updating MAP_SESSION");
                return StatusCode(500, new 
                { 
                    status = "fail", 
                    message = "Database error occurred while creating/updating MAP_SESSION record",
                    error = ex.InnerException?.Message ?? ex.Message 
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error creating/updating MAP_SESSION");
                return StatusCode(500, new 
                { 
                    status = "fail", 
                    message = "An error occurred while creating/updating MAP_SESSION record",
                    error = ex.Message 
                });
            }
        }
        ...
        public async Task<IActionResult> CreateSessionWithResults([FromBody] Models.DTOs.CreateSessionWithResultsRequest request)
        {
            ...
            {
                // Auto-populate createdBy if not provided
                var createdBy = string.IsNullOrWhiteSpace(request.Session.CreatedBy) ? "MAPTOOL" : request.Session.CreatedBy;
                ...
                // UPSERT MAP_SESSION: Check if session with SessionLink exists
                Models.Adamo.MapSession? mapSession = null;
                bool isSessionUpdate = false;

                if (!string.IsNullOrWhiteSpace(request.Session.SessionLink))
                {
                    mapSession = await _adamoContext.MapSessions
                        .FirstOrDefaultAsync(s => s.SessionLink == request.Session.SessionLink);
                    ...
                    if (mapSession != null)
                    {
                        ...
                        // Update existing session (only update non-null values)
                        if (request.Session.Stage != null) mapSession.Stage = request.Session.Stage;
                        if (request.Session.EvaluationDate.HasValue) mapSession.EvaluationDate = request.Session.EvaluationDate;
                        if (request.Session.Region != null) mapSession.Region = request.Session.Region;
                        if (request.Session.Segment != null) mapSession.Segment = request.Session.Segment;
                        if (request.Session.Participants != null) mapSession.Participants = request.Session.Participants;
                        if (request.Session.ShowInTaskList != null) 
                            mapSession.ShowInTaskList = request.Session.ShowInTaskList.ToUpper();
                        if (request.Session.SubStage.HasValue) mapSession.SubStage = request.Session.SubStage;
                        if (request.Session.Category != null) mapSession.Category = request.Session.Category;
                ...
                // Create new MAP_SESSION if not found
                if (mapSession == null)
                {
                    mapSession = new Models.Adamo.MapSession
                    {
                        ...
                        SessionLink = request.Session.SessionLink,
                        CreatedBy = createdBy,
                        CreationDate = DateTime.UtcNow,
                        LastModifiedDate = DateTime.UtcNow,
                        LastModifiedBy = createdBy
                ...
                _logger.LogInformation("{Action} MAP_SESSION with ID {SessionId}, SessionLink: {SessionLink}", 
                    isSessionUpdate ? "Updated" : "Created", mapSession.SessionId, mapSession.SessionLink);
                ...
                _logger.LogInformation("{Action} MAP_SESSION {SessionId} with {Count} MAP_RESULT records", 
                    isSessionUpdate ? "Updated" : "Created", mapSession.SessionId, mapResults.Count);

                Console.WriteLine($"✓ {(isSessionUpdate ? "Updated" : "Created")} MAP_SESSION: ID {mapSession.SessionId} (Link: {mapSession.SessionLink}) with {mapResults.Count} results");
                ...
                var actionMessage = isSessionUpdate 
                    ? $"MAP_SESSION updated with {mapResults.Count} results successfully"
                    : $"MAP_SESSION created with {mapResults.Count} results successfully";
